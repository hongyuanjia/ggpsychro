...
ggplot_add.CoordPsychro <- function(object, plot, object_name, ...) {
    ...
    if (is.null(object$units)) {
        ...
        message(sprintf(
            "Unit system has been reset from '%s' to '%s'.",
            plot$psychro$units,
            object$units
        ))
    ...
    if (is.null(object$limits$tdb)) {
        ...
        if (!is.null(plot$psychro$tdb_lim)) {
            ...
            message(sprintf(
                ...
                paste(plot$psychro$tdb_lim, collapse = ", "),
                u,
                paste(object$limits$tdb, collapse = ", "),
                u
            ))
        }
    ...
    if (is.null(object$limits$hum)) {
        ...
        if (!is.null(plot$psychro$hum_lim)) {
            ...
            message(sprintf(
                ...
                paste(plot$psychro$hum_lim, collapse = ", "),
                u,
                paste(object$limits$hum, collapse = ", "),
                u
            ))
        }
...
plot_add__comfort_foreground <- function(object, plot) {
    ...
    plot$coordinates$comfort_foreground <- c(
        plot$coordinates$comfort_foreground,
        list(object)
    )
    plot
}
...
ggplot_add.PsyScale <- function(object, plot, object_name, ...) {
    ...
    if (is.empty_trans(object$trans)) {
        # assign new trans
        trans <- get(
            paste0(object$scale_name, "_trans"),
            envir = asNamespace("ggpsychro")
        )
    ...
    if (identical(object$scale_name, "drybulb")) {
        object$aesthetics <- if (plot$psychro$mollier) {
            GGPSY_OPT$y_aes
        } else {
            GGPSY_OPT$x_aes
        }
