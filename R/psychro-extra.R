...
GetHumRatioFromAirVolume <- function(TDryBulb, AirVolume, Pressure) {
    if (isIP()) {
        ...
        HumRatio <- (AirVolume *
            (144 * Pressure) /
            (r_da * GetTRankineFromTFahrenheit(TDryBulb)) -
            1) /
            1.607858
    } else {
        r_da <- 287.042
        HumRatio <- (AirVolume *
            Pressure /
            (r_da * GetTKelvinFromTCelsius(TDryBulb)) -
            1) /
            1.607858
    }
...
GetTDryBulbFromMoistAirVolumeAndHumRatio <- function(
    MoistAirVolume,
    HumRatio,
    Pressure
) {
    if (psychrolib::isIP()) {
        R_DA_IP <- get(
            "R_DA_IP",
            envir = asNamespace("psychrolib"),
            inherits = FALSE
        )
        psychrolib::GetTFahrenheitFromTRankine(
            MoistAirVolume *
                (144 * Pressure) /
                (R_DA_IP * (1 + 1.607858 * HumRatio))
        )
    } else {
        R_DA_SI <- get(
            "R_DA_SI",
        ...
        psychrolib::GetTCelsiusFromTKelvin(
            MoistAirVolume * Pressure / (R_DA_SI * (1 + 1.607858 * HumRatio))
        )
    }
}

GetHumRatioFromMoistAirVolumeAndTDryBulb <- function(
    MoistAirVolume,
    TDryBulb,
    Pressure
) {
    if (psychrolib::isIP()) {
        ...
        (MoistAirVolume *
            (144 * Pressure) /
            psychrolib::GetTRankineFromTFahrenheit(TDryBulb) /
            R_DA_IP -
            1L) /
            1.607858
    } else {
        ...
        (MoistAirVolume *
            Pressure /
            psychrolib::GetTKelvinFromTCelsius(TDryBulb) /
            R_DA_SI -
            1L) /
            1.607858
    }
