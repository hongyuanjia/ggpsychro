...
bands, labelled PMV curves, and optional PMV-based standard zones.
...
standard layer draws the filled zone, PMV boundary curves, and labels.
...
boundaries. `geom_comfort_contour(label = TRUE)` draws the contour and its
inline level label as one layer, leaving a gap in the contour where the label is
placed.
...
Use `zone_style` with `element_comfort_zone()` to highlight individual strategy
