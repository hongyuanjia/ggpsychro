...
with `geom_comfort_givoni()`. The default fixed variant uses the commonly cited
1979 comfort anchor of 20 to 25.5 degrees C and 20% to 80% relative humidity,
with the hot-humid corner clipped. For project- or climate-specific assumptions,
set `tdb_range` and `relhum_range` on the fixed variant to use those ranges as
the drawing anchor. Use `variant = "adaptive"` and supply `mean_outdoor` to shift
the comfort anchor from mean outdoor temperature; the adaptive layer shows that
temperature as a dashed marker.
...
```{r givoni-zones, fig.alt = "Psychrometric chart with fixed Givoni-Milne strategy zones."}
...
```{r givoni-adaptive, fig.alt = "Psychrometric chart with adaptive Givoni-Milne strategy zones and a mean outdoor temperature marker."}
