...
`geom_comfort_givoni()` draws a Givoni-Milne strategy overlay, the
optional mean outdoor-temperature marker for adaptive strategies, and
...
- zone_style:
  ...
  Optional named list of per-zone style overrides. Names must match
  Givoni zone ids such as `"comfort"`, `"winter"`, or
  `"air_conditioning"`. Values can be created with
