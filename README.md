...
distribution plots. Tiles use a small default gap, partial transparency,
and a tile-aware cell grid across the chart area so the bin system is
visible even where no observations fall. Use `binwidth` and `boundary`
to define the dry-bulb and humidity-ratio cells, and `gap`, `alpha`,
`cell.grid.*`, `colour`, and `linewidth` to tune their appearance.
